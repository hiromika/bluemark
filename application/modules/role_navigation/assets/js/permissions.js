...
function formatCheck(v) {
    ...
    if(obj.value > 0){
        var c = '<input onclick="checkClick()" type="checkbox" checked="true" disabled role_id="'+obj.role+'" permission_id="'+obj.permission+'">'; 
    } else {
        var c = '<input onclick="checkClick(this)" type="checkbox" disabled role_id="'+obj.role+'" permission_id="'+obj.permission+'">';
    }
...
function disableRow(v) {
    ...
    $(tr).find('input').each (function(i,a) {
        ...
        output.push({"role_id":role,"navigation_id":permission,"value":value});
...
function install() {
    var row = $('#dg_module').treeGrid('getSelected');
    if (row) {
