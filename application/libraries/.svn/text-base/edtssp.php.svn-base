...
class EDTSSP {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = EDTSSP::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = EDTSSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
    ...
    {
        $bindings = array();
        $db = EDTSSP::sql_connect( $sql_details );

        // Build the SQL query string from the request
        $limit = EDTSSP::limit( $request, $columns );
        $order = EDTSSP::order( $request, $columns, $joinQuery );
        $where = EDTSSP::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            ...
            $col = EDTSSP::pluck($columns, 'db', $joinQuery);
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", EDTSSP::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = EDTSSP::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $resFilterLength = EDTSSP::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );
        ...
        $resTotalLength = EDTSSP::sql_exec( $db,$count_request);
        ...
        return array(
            ...
            "data"            => EDTSSP::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            EDTSSP::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
